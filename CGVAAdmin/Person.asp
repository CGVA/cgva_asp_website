 ...
 function validateInsert()
{
	...
	if (INSERT.EMAIL.value != "") {
	    if (ValidateEmail(INSERT.EMAIL.value) == false) {
	        INSERT.EMAIL.focus();
	        return false;
	    }
	}
...
//			{
				...
				{
					if(ValidateEmail(MODIFY.EMAIL[selectedValue].value) == false)
					{
						//alert("Please enter a last name for the record being modified.");
						MODIFY.EMAIL[selectedValue].focus();
						return false;
					}
				}
...
function disableCheck(obj)
{
	...
	var LASTDIG_IND = eval("MODIFY.LASTDIG_IND" + rowValue);
	var LOGICAL_DELETE_IND = eval("MODIFY.LOGICAL_DELETE_IND" + rowValue);
...
<td valign='top'>
	...
	Sub chooseOption()
		If Now >= "8/25/2009 12:00:00" And Now <= "8/25/2009 21:00:00" Then
			rw("<div align='center'>Today's date/time: " & Now & "</div>")
		End If
		%>
		...
		<td>
			<font class='cfont10'>
			<input type="radio" name="choice" value="modify" onClick="CHOICE.FILTER.focus();"> Modify Existing Person
			&nbsp;
			<select name='FILTER' onChange="CHOICE.choice[1].checked = true;">
	...
	''		End If
		...
		<tr>
		<td colspan='22'><input type='submit' name="submitChoice" value='Modify Person(s)' onClick="return validateModify();"></td>
		</tr>
		...
		%>
					...
					<th valign='bottom'><font class='cfontWhite10'><b>Last Dig Only?</b></font></th>
					<th valign='bottom'><font class='cfontWhite10'><b>Inactive?</b></font></th>
					<th valign='bottom'><font class='cfontWhite10'><b>Ref Rec?</b></font></th>
					</tr>
		<%
		...
		%>
				...
				<tr data-row="<%=rowCount%>"bgcolor="<%=BGCOLOR%>">
				<td align='center'><input type="hidden" id="ID<%=rs("PERSON_ID")%>" name="ID" value="<%=rs("PERSON_ID")%>" /><font class='cfont10'><%=rs("PERSON_ID")%></font></td>
				<td><input type="text" id="FIRST_NAME<%=rs("PERSON_ID")%>" name="FIRST_NAME" value="<%=rs("FIRST_NAME")%>" size='25' maxlength='25'></td>
				<td><input type="text" id="LAST_NAME<%=rs("PERSON_ID")%>" name="LAST_NAME" value="<%=rs("LAST_NAME")%>" size='25' maxlength='35'></td>
				<td><input type="text" id="PHOTO_FILENAME<%=rs("PERSON_ID")%>" name="PHOTO_FILENAME" value="<%=rs("PHOTO_FILENAME")%>" size='20' maxlength='100'></td>
				<td><input type="text" id="EMAIL<%=rs("PERSON_ID")%>" name="EMAIL" value="<%=rs("EMAIL")%>" size='30' maxlength='50'></td>
				<td><input type="text" id="BIRTH_DATE<%=rs("PERSON_ID")%>" name="BIRTH_DATE" value="<%=rs("BIRTH_DATE")%>" size='10' maxlength='10'></td>

				<td>
					<select id="GENDER<%=rs("PERSON_ID")%>" name="GENDER">
					<%
						rw("<option value='M' ")
				...
				<td><input type="text" id="PRIMARY_PHONE_NUM<%=rs("PERSON_ID")%>" name="PRIMARY_PHONE_NUM" value="<%=rs("PRIMARY_PHONE_NUM")%>" size='10' maxlength='10'></td>
				<td><input type="text" id="PHONE2<%=rs("PERSON_ID")%>" name="PHONE2" value="<%=rs("PHONE2")%>" size='10' maxlength='10'></td>
				<td><input type="text" id="PHONE3<%=rs("PERSON_ID")%>" name="PHONE3" value="<%=rs("PHONE3")%>" size='10' maxlength='10'></td>

				<td><input type="text" id="ADDRESS_LINE1<%=rs("PERSON_ID")%>" name="ADDRESS_LINE1" value="<%=rs("ADDRESS_LINE1")%>" size='30' maxlength='40'></td>
				<td><input type="text" id="ADDRESS_LINE2<%=rs("PERSON_ID")%>" name="ADDRESS_LINE2" value="<%=rs("ADDRESS_LINE2")%>" size='30' maxlength='40'></td>

				<td><input type="text" id="CITY<%=rs("PERSON_ID")%>" name="CITY" value="<%=rs("CITY")%>" size='20' maxlength='30'></td>
				<td><input type="text" id="STATE<%=rs("PERSON_ID")%>" name="STATE" value="<%=rs("STATE")%>" size='2' maxlength='2'></td>
				<td><input type="text" id="ZIP<%=rs("PERSON_ID")%>" name="ZIP" value="<%=rs("ZIP")%>" size='10' maxlength='10'></td>

				<td><input type="text" id="EMERGENCY_FIRST_NAME<%=rs("PERSON_ID")%>" name="EMERGENCY_FIRST_NAME" value="<%=rs("EMERGENCY_FIRST_NAME")%>" size='25' maxlength='25'></td>
				<td><input type="text" id="EMERGENCY_LAST_NAME<%=rs("PERSON_ID")%>" name="EMERGENCY_LAST_NAME" value="<%=rs("EMERGENCY_LAST_NAME")%>" size='25' maxlength='35'></td>
				<td><input type="text" id="EMERGENCY_PHONE<%=rs("PERSON_ID")%>" name="EMERGENCY_PHONE" value="<%=rs("EMERGENCY_PHONE")%>" size='10' maxlength='10'></td>

				<td>
					<select id="SUPPRESS_EMAIL_IND<%=rs("PERSON_ID")%>" name="SUPPRESS_EMAIL_IND">
					<%
						rw("<option value='N' ")
				...
				<td>
					<select id="SUPPRESS_SNAIL_MAIL_IND<%=rs("PERSON_ID")%>" name="SUPPRESS_SNAIL_MAIL_IND">
					<%
						rw("<option value='N' ")
				...
				<td>
					<select id="SUPPRESS_LAST_NAME_IND<%=rs("PERSON_ID")%>" name="SUPPRESS_LAST_NAME_IND">
					<%
						rw("<option value='N' ")
				...
				<td>
					<select id="FIRST_CONTACT_ID<%=rs("PERSON_ID")%>" name="FIRST_CONTACT_ID">
				...
				<td><input type="text" id="NAGVA_RATING<%=rs("PERSON_ID")%>" name="NAGVA_RATING" value="<%=rs("NAGVA_RATING")%>" size='10' maxlength='50'></td>
				<td>
					<select id="LASTDIG_IND<%=rs("PERSON_ID")%>" name="LASTDIG_IND">
					<%
						rw("<option value='N' ")

						If rs("LASTDIG_IND") = "N" then
							rw("selected")
						End If
				...
				<td>
					...
					<select id="LOGICAL_DELETE_IND<%=rs("PERSON_ID")%>" name="LOGICAL_DELETE_IND">
					<%
						rw("<option value='N' ")
				...
				<td>
					<!-- <span><% rw(IsNull(rs("REF_REC"))) %></span>-->

					<input  name="REF_REC" type="checkbox" 
					<%
						If Not IsNull(rs("REF_REC")) then
						rw("checked='checked'")
						End if
	...
	%>
		...
		<tr>
		<td align='right' valign='middle'><font size='2' face='arial'><b>First Name</b></font><font class="cfontError10">*</font></td>
		<td><input type="text" name='FIRST_NAME' size='25' maxlength='25'></td>
		</tr>

		<tr>
		<td align='right' valign='middle'><font size='2' face='arial'><b>Last Name</b></font><font class="cfontError10">*</font></td>
		<td><input type="text" name='LAST_NAME' size='25' maxlength='35'></td>
		</tr>

		<tr>
		<td align='right' valign='middle'><font size='2' face='arial'><b>Email</b></font><font class="cfontError10">*</font></td>
		...
		<tr>
		<td align='right' valign='middle'><font size='2' face='arial'><b>First Contact?</b></font><font class="cfontError10">*</font></td>
		<td>
		...
		<tr>
		<td align='right' valign='middle'><font size='2' face='arial'><b>Last Dig Only?</b></font></td>
		<td>
			<select name='LASTDIG_IND'>
			<option value='N'>No</option>
			<option value='Y'>Yes</option>
			</select>
		</td>
