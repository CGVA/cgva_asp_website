...
function validateModify()
{
	...
	if(MODIFY.ID.length > 1)
	{
		...
		for(i = 0;i < MODIFY.ID.length;i++)
		{
			...
			if(MODIFY.ID[i].checked)
			{
				...
				if(!isInteger(MODIFY.WEEK_NUM[selectedValue].value))
				{
					alert("Please enter a numeric week number.");
					MODIFY.WEEK_NUM[selectedValue].focus();
					return false;
	...
	else
	{
		...
		if(!isInteger(MODIFY.WEEK_NUM.value))
		{
...
<td valign='top'>
	...
	''		End If
		...
		%>
				...
				<td><input type="text" id="WEEK_NUM<%=rs("WEEK_ID")%>" name="WEEK_NUM" value="<%=rs("WEEK_NUM")%>" size='3' maxlength='2' disabled></td>
	...
	%>
		...
		<td><input type="text" name='WEEK_NUM' size='3' maxlength='2'></td>
		</tr>
